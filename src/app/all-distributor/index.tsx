...
export default function AllDistributorScreen() {
  ...
  const total = distributorsData?.total ?? 0;
  ...
  const handleExportXlsx = async () => {
    try {
      ...
      const rowMapper = (u: UserWithoutPassword) => ({
        'Distributor Name': u.name,
        'Entity ID': u.entityId,
        'Email': u.email || '-',
        'Phone': u.phone || '-',
        'State': u.state || '-',
        'Beat/Territory': u.beat || '-',
      ...
      await exportPaginatedData(
        ...
        rowMapper,
        {
          fileName: `all-distributors-${new Date().toISOString().slice(0, 10)}.xlsx`,
          sheetName: 'Distributors',
        }
      );
    } catch (err: any) {
      console.error('Export Distributor error:', err);
      Alert.alert('Export Error', err?.message || 'Failed to export Distributor data');
    } finally {
        ...
        <View className="flex-row items-center gap-3 flex-1">
          ...
          <View className="flex-1">
            <View className="flex-row items-center gap-2">
              <Text className="text-lg font-bold text-gray-800">All Distributors</Text>
            ...
            <Text className="text-xs text-gray-400 mt-0.5" numberOfLines={1}>
              Complete list & data export of registered Distributors
            </Text>
          </View>
        </View>
      ...
      >
        ...
        <View className="p-4 m-4 bg-white border border-gray-200 rounded-2xl shadow-sm gap-4">
          ...
          <View className="gap-1.5">
            <Text className="text-xs font-bold text-slate-500 uppercase tracking-wider">Search Distributor</Text>
        ...
        <View className="px-4 pb-20 gap-3">
          ...
          {isLoading ? (
            <View className="bg-white border border-gray-200 rounded-2xl p-12 items-center justify-center shadow-sm">
              ...
              <Text className="text-xs text-gray-500 mt-3 font-medium">Loading Distributor data...</Text>
            </View>
          ) : distributors.length === 0 ? (
            <View className="bg-white border border-gray-200 rounded-2xl p-12 items-center justify-center shadow-sm">
              <Ionicons name="people-outline" size={40} color="#9ca3af" className="opacity-40" />
              <Text className="text-sm font-semibold text-gray-600 mt-3">No Distributors Found</Text>
              <Text className="text-xs text-gray-400 mt-1 text-center">
                No Distributor accounts match your current search and state filter.
              </Text>
            </View>
          ) : (
            <View className="gap-3">
              {distributors.map((u: UserWithoutPassword) => (
                <View
                  key={u.entityId}
                    ...
                    <View className="flex-1 pr-2">
                      <View className="flex-row items-center gap-2 mb-1">
                        ...
                        <View className="bg-orange-100 border border-orange-200 px-2 py-0.5 rounded-full">
                          <Text className="text-[9px] font-bold text-orange-700">DISTRIBUTOR</Text>
                        </View>
                      </View>
